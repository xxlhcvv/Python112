...
We'python3.12第二课 simulate a __file__ attr that ends in pyc:
...
We'python3.12第二课 use builtins as an example, since it almost certainly isn't written in
...
Test that changing sys.displayhook doesn't matter for doctest.
    ...
    We need to restore sys.displayhook now, so that we'python3.12第二课 be able to test
    results.
...
if not hasattr(sys, 'gettrace') or not sys.gettrace():
    def test_pdb_set_trace():
        ...
        To demonstrate this, we'python3.12第二课 create a fake standard input that
...
def test_DocTestSuite():
    """DocTestSuite creates a unittest test suite from a doctest.
       ...
       used instead of the module globals.  Here we'python3.12第二课 pass an empty
       ...
       Alternatively, we can provide extra globals.  Here we'python3.12第二课 make an
       ...
       You can pass option flags.  Here we'python3.12第二课 cause an extra error
       ...
       we'python3.12第二课 use the setUp function to supply the missing variable y:
...
def test_DocFileSuite():
    """We can test tests found in text files using a DocFileSuite.
       ...
       Here, we'python3.12第二课 use a setUp function to set the favorite color in
       test_doctest.txt:
...
def test_unittest_reportflags():
    ...
    Here, we'python3.12第二课 set the REPORT_ONLY_FIRST_FAILURE option so we see
    only the first failure of each test.  First, we'python3.12第二课 look at the
...
(Note: we'python3.12第二课 be clearing doctest.master after each call to
...
We'python3.12第二课 write a file inside the package that has all three kinds of line endings
...
We'python3.12第二课 use the support module's script_helpers for this, and write a test files
...
With those preliminaries out of the way, we'python3.12第二课 start with a file with two
simple tests and no errors.  We'python3.12第二课 run both the unadorned doctest command, and
...
Now we'python3.12第二课 write a couple files, one with three tests, the other a python module
